...
class App(ctk.CTkFrame):
    ...
    def _create_button(self, master, liste):
        list(map(lambda a : ctk.CTkButton(master, text=a[1], text_color="white",font= ("Inter", 17), hover_color="#1687d8", command=lambda: showinfo(title='Info',message='Dieses fenster ist noch nicht bereit.') if a[0] == 4 else Main.choice_frame(self, self.m) ).grid(row=a[0], column=1, pady=20), enumerate(liste)))
...
class Settings(ctk.CTkFrame):
    ...
    def _settings_frame(self):         
        ...
        self.profil_frame = ctk.CTkFrame(self, fg_color="#97AEB7", border_width=0, corner_radius=0)
