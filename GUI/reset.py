...
def create_reset_frame(parent, commando):
    ...
    username_entry = ctk.CTkEntry(email_frame, width=300, height=30, corner_radius=10)
    ...
    email_entry = ctk.CTkEntry(email_frame, width=300, height=30, corner_radius=10)
    ...
    secret_code_entry = ctk.CTkEntry(email_frame, width=300, height=30, corner_radius=10)
    secret_code_entry.grid(row=2, column=1, padx=10, pady=20)

    #button to reset password
    reset_button = ctk.CTkButton(reset_frame, text="Reset Password", command= commando, width=150, height=40, corner_radius=10)   
    ...
    back_button = ctk.CTkButton(back_frame, text="Login", command=lambda: print("Back to Login clicked"), fg_color="#333", text_color="#19C")
    back_button.grid(row=0, column=1)
    or_label = ctk.CTkLabel(back_frame, text="or", text_color="white", font=("Arial", 13))
    or_label.grid(row=0, column=2)
    # registration button
    registrierung_btton = ctk.CTkButton(back_frame, text="Register", command=lambda: print("Register clicked"), fg_color="#333", text_color="#19C")
...
if __name__ == "__main__":
    ...
    frame = create_reset_frame(app, lambda: print("connected"))
